...
public class ResumeDaoTest {
    ...
    public void testFindById() {
        ...
        // resume0_.name as name3_0_0_, resume0_.phone as phone4_0_0_ from tb_resume resume0_ where resume0_.id=?
    ...
    public void testSave() {
        // 新增和更新都是用Save方法，通过传入对象的主键有误来区分，没有主键信息就是新增，有主键信息就是更新
        Resume resume = new Resume();
        resume.setId(5l);
        resume.setName("赵六六");
        resume.setAddress("成都");
        resume.setPhone("138000000");
     ...
     * 方式二：可以引入jpql (jpa查询语句) 语句进行查询 (=====>>>> jpql语句类似sql，只不过sql操作的是数据库表和字段，jpql操作的是对象和属性，比如 from Resume where id = xx) hql
     * 方式三：引入原生的sql语句
     * 方式四：可以在接口中自定义方法，而且不必引入jpql或者sql语句，这种方式叫做方法命名规则查询，也就是说定义接口方法名是按照一定规则形参，那么框架就会理解意图
     * 方法五：动态查询
    ...
    @Test
    public void testSql() {
        List<Resume> resumeList = resumeDao.findBySql("李%", "上海%");
        for (int i = 0; i < resumeList.size(); i++) {
            Resume resume = resumeList.get(i);
            System.out.println(resume);
        }
    ...
    public void testMethodName() {
        List<Resume> resumeList = resumeDao.findByNameLikeAndAddress("李%", "上海");
        for (int i = 0; i < resumeList.size(); i++) {
            Resume resume = resumeList.get(i);
            System.out.println(resume);
        }
    ...
    public void testSpecification() {
         ...
         *      借助两条参数完成条件封装：   select * from tb_resume where name = '张三'
         *          Root：获取需要查询的条件属性
    ...
    public void testSpecficationMultiCon() {
        /**
         *      需求：根据name (指定为"张三") 查询简历，且address以"北"开头（模糊查询），查询简历
         */
        Specification<Resume> specification = new Specification<Resume>() {
            @Override
            public Predicate toPredicate(Root<Resume> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder) {
